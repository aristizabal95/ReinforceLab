...
if __name__ == "__main__":
    env = gym.make("Ant-v4", render_mode="rgb_array", healthy_reward=1)
    ...
    # train(env, agent, path, epsilon=1., num_epochs=25000, epsilon_decay=3.5e-7, min_epsilon=0.1)
    agent.load(path)
    env = gym.make("Ant-v4", render_mode="human", healthy_reward=1)
    test(env, agent)
